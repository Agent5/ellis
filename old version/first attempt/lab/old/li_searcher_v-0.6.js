...
this.echo('Logged in', 'debug');
  ...
  self.thenOpen(usableUri, function() {
    this.waitForSelector('li.entity', function() {
      ...
      var jsonFileContents = fs.open('results/json/LI_results_v-0.6.json', 'r');
      ...
      if(index < 1) {
        ...
        fs.write('results/json/LI_results_v-0.6.json', '[\n' + jsonResults, 'a');
        ...
        jsonResults = totalSet;
          fs.write('results/json/LI_results_v-0.6.json', ',\n' + jsonResults + '\n]', 'a');
        this.echo('No: ' + (index + 1) + ', ' + colorizer.colorize('Name: ', 'INFO') + link.target + ', ' + colorizer.colorize('Company: ', 'INFO') + link.company + ', ' + colorizer.colorize('ID: ', 'INFO') + link.id + '\nCompleting the file.\nGoodbye.\n');
      }// if LI_results_v-0.6.json is empty then append a '[', else append a ',' plus the data
