...
var links = [
    {uri: "https://www.linkedin.com/sales/search?keywords=Ryan+Williams+Pearle+Vision+Jacksonville", target: "Ryan Williams", company: "Pearle Vision Jacksonville", id: "00Qi000000feJFK", num: "001"},
    {uri: "https://www.linkedin.com/sales/search?keywords=Brian+Stephens+Dell", target: "Brian Stephens", company: "Dell", id: "00Qi000000gjov5", num: "002"},
    {uri: "https://www.linkedin.com/sales/search?keywords=Gregory+Estes+Monrovia+Growers", target: "Gregory Estes", company: "Monrovia Growers", id: "00Qi000000gkLJK", num: "003"},
    {uri: "https://www.linkedin.com/sales/search?keywords=Ramiro A+Ramirez+Link+Engineering+Company", target: "Ramiro A Ramirez", company: "Link Engineering Company", id: "00Qi000000gkLJL", num: "004"},
        ...
        this.waitForSelector('li.entity', function() {
            ...
            var results = this.evaluate(function() {
              var entityResults = [];
                ...
                entityFormatter = JSON.stringify(entityResults);
                // console.table(results);
                return entityFormatter;
            });

            var pedigreeData = '<table class="pedigree" id="pedigree"><thead><td>No.</td><td>Name</td><td>Title</td><td>Company</td><td>Lead ID</td></thead><td>' + link.num + '</td><td>' + link.target + '</td><td>&nbsp;</td><td>' + link.company + '</td><td>' + link.id + '</td></table>';
            var pedigreeEcho = 'No: ' + link.num + ', Name: ' + link.target + ', Company: ' + link.company + ', ID: ' + link.id;
            var formattedResults = pedigreeData + results;

            fs.write('results/html/' + fileName + '.html', formattedResults, 'w');
            var jsonFile = fs.open('results/json/LI_results.json', 'r');
            var jsonResults = jsonFile;
            jsonResults = jsonResults + results;
            fs.write('results/json/LI_results.json', jsonResults, 'a');
            this.echo(pedigreeEcho);
