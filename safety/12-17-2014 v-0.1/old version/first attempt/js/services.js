...
function Item(entry) {
  ...
  if (entry) {
		this.lead_name = entry.lead_name;
	  this.item_id = entry.num;
	  this.key = this.sfdc_id; // For LawnChair.
	  this.pub_name = entry.target_company; // Set the pub name to the feed's title.
	  this.pub_author = entry.lead_source;
	  this.pub_date = entry.date_created; // new Date();
    this.title = entry.linkSearchResults[0].title;
	  this.item_link = entry.linkSearchResults[0].link;
	  this.feed_link = entry.linkSearchResults[0].link;
	  this.content = entry.first_name + '\n' + entry.sfdc_id;
...
services.factory('items', ['$http', 'store', 'filterFilter', function($http, store, filter) {
	var items = {
      ...
      var feedURL = 'http://10.0.1.116:3000/';
      ...
      $http.get('http://10.0.1.116:3000/entry').success(successCallback);
      $http.get('http://10.0.1.116:3000/entry').success(function(data, status, headers, config) {
        $scope.posts = data;
      }).
