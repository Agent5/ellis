    ...
    this.waitForSelector('li.entity', function() {
      ...
      var results = this.evaluate(function() {
        ...
        $('.entity').each(function(searchIndex){
          ...
          // leadInfoSet.searchTarget = hellBent;
      ...
      personResults.leadName = leadName;
      personResults.searchTarget = searchTarget;
      personResults.sfdcID = sfdcID;
      personResults.searchTime = '';
      personResults.num = num;
      personResults.target_company = target_company;

      var totalSet = JSON.stringify(personResults, null, '    ');

      var jsonFileContents = fs.open('results/json/LI_results_v-0.8_EXPERIMENTAL.json', 'r');
      ...
      if(index < 1) {
        ...
        fs.write('results/json/LI_results_v-0.8_EXPERIMENTAL.json', '{\n    \"entry\": [\n' + jsonResults, 'a');
        ...
        fs.write('results/json/LI_results_v-0.8_EXPERIMENTAL.json', ',\n' + jsonResults + '\n]', 'a');
        this.echo('No: ' + (index + 1) + ', ' + colorizer.colorize('Name: ', 'INFO') + link.target + ', ' + colorizer.colorize('Company: ', 'INFO') + link.company + ', ' + colorizer.colorize('ID: ', 'INFO') + link.id + '\nCompleting the file.\nGoodbye.\n');
      }// if LI_results_v-0.8_EXPERIMENTAL.json is empty then append a '[', else append a ',' plus the data
