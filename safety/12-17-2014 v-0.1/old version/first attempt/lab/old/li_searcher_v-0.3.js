    ...
    self.thenOpen(usableUri, function() {
        this.waitForSelector('li.entity', function() {
            ...
            var results = this.evaluate(function() {
                  var resultsHTML = $('ul.results').html();
                  return resultsHTML;
            });
            ...
            var pedigreeEcho = 'No: ' + link.num + ', Name: ' + link.name + ', Company: ' + link.company + ', ID: ' + link.id;
            var formattedResults = pedigreeData + results;
            this.echo(pedigreeEcho);

            fs.write('results/' + fileName + '.html', formattedResults, 'w');
