...
this.echo('Logged in', 'debug');
  ...
  self.thenOpen(usableUri, function() {
    this.waitForSelector('h3.company-name', function() {
      ...
      var jsonFileContents = fs.open('results/json/li_get_current_employer._v-0.2.json', 'r');
      ...
      if(index < 1) {
        ...
        fs.write('results/json/li_get_current_employer._v-0.2.json', '[\n' + jsonResults, 'a');
        ...
        fs.write('results/json/li_get_current_employer._v-0.2.json', ',\n' + jsonResults + '\n]', 'a');
        this.echo('No: ' + (index + 1) + ', ' + colorizer.colorize('Name: ', 'INFO') + link.target + ', ' + colorizer.colorize('Company: ', 'INFO') + link.company + ', ' + colorizer.colorize('ID: ', 'INFO') + link.id + '\nCompleting the file.....  \nGoodbye.\n');
      }// if li_get_current_employer._v-0.2.json is empty then append a '[', else append a ',' plus the data
